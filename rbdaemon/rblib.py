...
#        return self._perform_session()
    ...
    def connectionOk(self):
        ...
        if not self._isNetworkTimeValid():
            # self.callback.on_status("Invalid network time",
            #                      RockBlockEventHandler.STATUS_ERROR)
            return False
        ...
        # if not signal > 0:
        #     self.callback.on_status("no signal", RockBlockProtocol.STATUS_ERROR)
        #     return False
        # else:
        #     return True
        return True
    ...
    def get_net_time(self):
        ...
        self.expect("OK")  # OK
        utc = 0
        if response != None and "no network service" not in response:
        #     self.callback.on_status("no network time response",
        #                          RockBlockEventHandler.STATUS_ERROR)
        # elif "no network service" in response:
        #     self.callback.on_status("no network service",
        #                          RockBlockEventHandler.STATUS_ERROR)
        # else:
            # self.callback.on_status("network time success",
    ...
    def write_mo_buffer(self, msg):
        ...
        # self.callback.on_status("adding message to buffer",
        #                      RockBlockEventHandler.STATUS_INFO)
        ...
        if self.expect("READY") != None:
            ...
            # self.callback.on_status("READY message not found",
            #                      RockBlockEventHandler.STATUS_ERROR)
            return False
    ...
    def perform_session(self):
        ...
        if response == None:
            # self.callback.on_status("invalid session response",
            #                      RockBlockEventHandler.STATUS_ERROR)
            return False
    ...
    def read_mt_buffer(self):
        ...
        # self.callback.on_status("Reading MT message",
        #                      RockBlockEventHandler.STATUS_INFO)
        ...
        if length > 0:
            ...
            # if mysum != cksum:
                # self.callback.on_status("checksum mismatch {:d} {:d}".format(
    ...
    def clear_mo_buffer(self):
        self._verify_serial_connected()
        # self.callback.on_status("clearing MO buffer",
        #                      RockBlockEventHandler.STATUS_INFO)
        ...
        self.serial_readline()  # OK
        if r1 == None:
            self.callback.on_status("clear buffer: {} expected 0".format(
    ...
    def clear_mt_buffer(self):
        self._verify_serial_connected()
        # self.callback.on_status("clearing MT buffer",
        #                      RockBlockEventHandler.STATUS_INFO)
    ...
    def mo_status_to_string(self, err):
        ...
        #     self.callback.on_status("key error {}".format(
