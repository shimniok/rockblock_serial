...
class RockBlock(object):
    ...
    def messageCheck(self):
        self.callback.status("Message RX started...", RockBlockProtocol.STATUS_INFO)
        if self.connectionOk():
            self._perform_session()

    def sendMessage(self, msg):
        self.callback.status("Message TX started...", RockBlockProtocol.STATUS_INFO)
        if not self.connectionOk():
            return False

        if not self._queueMessage(msg):
            self.callback.status("queue message failed", RockBlockProtocol.STATUS_ERROR)
            return False
    ...
    def connectionOk(self):
        ...
        if not self._isNetworkTimeValid():
            self.callback.status("Invalid network time", RockBlockProtocol.STATUS_ERROR)
            return False
    ...
    def requestSignalStrength(self):
        ...
        try:
            ...
            signal = 0
        self.callback.status("Signal: {:d}".format(signal), RockBlockProtocol.STATUS_INFO)
...
#        self.callback.signal_event(ev)
    ...
    def networkTime(self):
        ...
        utc = 0
        if response == None:
            self.callback.status("no network time response", RockBlockProtocol.STATUS_ERROR)
        elif "no network service" in response:
            self.callback.status("no network service", RockBlockProtocol.STATUS_ERROR)
        else:
            self.callback.status("network time success", RockBlockProtocol.STATUS_SUCCESS)
            utc = int(response, 16)
    ...
    def _queueMessage(self, msg):
        ...
        if len(msg) > 340:
            self.callback.status("message must be 340 bytes or less", RockBlockProtocol.STATUS_ERROR)
            return False
        ...
        if self.expect("READY") != None:
            ...
            self.callback.status("READY message not found", RockBlockProtocol.STATUS_ERROR)
            return False
    ...
    def _perform_session(self):
        ...
        if response == None:
            self.callback.status("invalid session response", RockBlockProtocol.STATUS_ERROR)
            return False
        ...
        if moStatus <= 4:
            self.callback.status("session succeeded", RockBlockProtocol.STATUS_SUCCESS)
            return True
        else:
            self.callback.status("session failed", RockBlockProtocol.STATUS_ERROR)
            return False

    def _read_mt_message(self):
        self._verify_serial_connected()

        self.callback.status("Reading MT message", RockBlockProtocol.STATUS_INFO)
    ...
    def _clearMoBuffer(self):
        self._verify_serial_connected()
        self.callback.status("clearing MO buffer", RockBlockProtocol.STATUS_INFO)
        self.send_command("AT+SBDD0")
        r1 = self.expect("0")
        self.serial_readline()  #BLANK
        self.serial_readline()  #OK
        if r1 == None:
            self.callback.status("clear buffer: {} expected 0".format(r1), RockBlockProtocol.STATUS_ERROR)
            return False
        else:
            return True
